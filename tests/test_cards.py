...
from pocketshell.cards import store as cards_store
...
def test_private_write_orders_durability_barriers_around_replace(
    ...
    events: list[str] = []
    real_fsync = cards_store.os.fsync
    real_replace = cards_store.os.replace
    ...
    monkeypatch.setattr(cards_store.os, "fsync", record_fsync)
    monkeypatch.setattr(cards_store.os, "replace", record_replace)
...
def test_private_write_fails_before_replace_when_file_fsync_fails(
    ...
    monkeypatch.setattr(cards_store.os, "fsync", fail_fsync)
    monkeypatch.setattr(cards_store.os, "replace", record_replace)
